...
namespace LiveLarson.Util
{
    public static class Extensions
    {
        ...
        public static string FormatToStringKM(this float value)
        {
            switch (value)
            {
                case < 1000:
                ...
                case < 1000000:
                {
                    var floatValue = value / 1000.0f;
                    return floatValue.ToString("0.0") + "K";
                }
                default:
                {
                    var floatValue = value / 1000000.0f;
                    return floatValue.ToString("0.0") + "M";
                }
            }
        }
        
        public static string FormatToKRW(this int value)
        {
            var nfi = new CultureInfo("ko-KR", false).NumberFormat;
            nfi.CurrencySymbol = "₩";
            var price = value.ToString("C0", nfi);
            return price;
        }
    }
}